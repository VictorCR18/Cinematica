...
export const listReviewsForMovie = (tmdbId: number, page = 1) =>
  api
    ...
    >(`/movies/${tmdbId}/reviews`, { params: { page } })
    .then((r) => r.data);

export const likeReview = (id: string) =>
  api.post(`/reviews/${id}/like`).then((r) => r.data);
export const unlikeReview = (id: string) =>
  api.delete(`/reviews/${id}/like`).then((r) => r.data);
export const deleteReview = (id: string) =>
  api.delete(`/reviews/${id}`).then((r) => r.data);
export const updateReview = (
  id: string,
  input: { content: string; rating?: number; containsSpoilers?: boolean },
) => api.patch<Review>(`/reviews/${id}`, input).then((r) => r.data);
