...
import { AuthHint } from "../components/ui/Authhint";
...
export const MovieDetailsPage = () => {
  ...
  const handleReviewDeleted = (reviewId: string) => {
    ...
    queryClient.setQueryData<typeof movie>(["movie", id], (old) =>
      old
        ? {
            ...old,
            stats: {
              ...old.stats,
              reviewsCount: Math.max(0, old.stats.reviewsCount - 1),
            },
          }
        : old,
    );
            ...
            <div className="mt-6 flex flex-wrap items-center gap-6">
              <div className="flex items-center gap-2">
                ...
                <span className="font-mono text-lg text-paper">
                  {movie.stats.averageRating?.toFixed(1) ?? "—"}
                </span>
                <span className="text-xs text-muted">
                  ({movie.stats.ratingsCount} avaliações no Cinemática)
                </span>
              </div>
            </div>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-4">
              <div>
                <p className="mb-1.5 text-xs text-muted">
                  {isAuthenticated ? "Sua nota" : "Entre para avaliar"}
                </p>
                <StarRating
              ...
              <Button
                ...
                onClick={() =>
                  requireAuth(
                    "Entre na sua conta para registrar filmes no seu diário.",
                    () => setLogOpen(true),
                  )
                }
              >
              ...
              {cast.map((member) => {
                ...
                return (
                  <div key={member.id} className="w-24 shrink-0 text-center">
                    ...
                    <p className="mt-2 text-xs font-medium text-paper line-clamp-1">
                      {member.name}
                    </p>
        ...
        <section>
          ...
          {reviews && reviews.data.length > 0 ? (
            ...
            <p className="text-sm text-muted">
              Nenhuma resenha ainda. Seja o primeiro a escrever uma!
            </p>
          )}
        </section>
