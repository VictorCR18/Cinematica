...
const cardVariants: Variants = {
  ...
  visible: (i: number) => ({
    opacity: 1,
    y: 0,
    transition: { delay: Math.min(i, 12) * 0.04, duration: 0.4, ease: [0.22, 1, 0.36, 1] as const },
  }),
};
      ...
      <Link to={`/filme/${movie.id}`} className="group block">
        ...
        >
          ...
          <div className="absolute inset-x-0 bottom-0 translate-y-2 p-3 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
            <p className="font-display text-sm font-semibold leading-snug text-paper line-clamp-2">{movie.title}</p>
