...
export const createReview = async (userId: string, tmdbId: number, input: CreateReviewInput) => {
  ...
  await logActivity({ type: 'REVIEW', userId, movieId: movie.id, refId: review.id });
  return review;
};

export const listReviewsForMovie = async (tmdbId: number, query: PaginationQuery) => {
...
export const getReviewById = async (id: string) => {
  const review = await prisma.review.findUnique({ where: { id }, include: { user: { select: authorSelect }, movie: true } });
  if (!review) throw AppError.notFound('Resenha não encontrada');
  return review;
};

export const updateReview = async (userId: string, id: string, input: Partial<CreateReviewInput>) => {
  const review = await prisma.review.findUnique({ where: { id } });
  if (!review) throw AppError.notFound('Resenha não encontrada');
  if (review.userId !== userId) throw AppError.forbidden('Você só pode editar suas próprias resenhas');

  return prisma.review.update({ where: { id }, data: input, include: { user: { select: authorSelect } } });
};

export const deleteReview = async (userId: string, id: string) => {
  const review = await prisma.review.findUnique({ where: { id } });
  if (!review) throw AppError.notFound('Resenha não encontrada');
  if (review.userId !== userId) throw AppError.forbidden('Você só pode remover suas próprias resenhas');
...
export const likeReview = async (userId: string, reviewId: string) => {
  ...
  const alreadyLiked = await prisma.reviewLike.findUnique({ where: { userId_reviewId: { userId, reviewId } } });
...
export const unlikeReview = async (userId: string, reviewId: string) => {
  ...
  await prisma.$transaction([
    ...
    prisma.review.update({ where: { id: reviewId }, data: { likesCount: { decrement: 1 } } }),
  ]);
