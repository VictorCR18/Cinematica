...
export const logMovie = async (userId: string, input: CreateDiaryInput) => {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    await logActivity({ type: 'DIARY', userId, movieId: movie.id, refId: entry.id });
    return entry;
  });
};
...
export const listMyDiary = async (userId: string, query: PaginationQuery) => {
  ...
  const [data, total] = await Promise.all([
    prisma.diaryEntry.findMany({ where: { userId }, include, orderBy: { watchedAt: 'desc' }, skip, take }),
    prisma.diaryEntry.count({ where: { userId } }),
  ]);
  return { data, meta: buildMeta(page, limit, total) };
};

export const listDiaryByUsername = async (username: string, query: PaginationQuery) => {
  const user = await prisma.user.findUnique({ where: { username } });
  if (!user) throw AppError.notFound('Usuário não encontrado');
  return listMyDiary(user.id, query);
};

export const updateEntry = async (userId: string, id: string, data: { watchedAt?: Date; rewatch?: boolean; rating?: number }) => {
  const entry = await prisma.diaryEntry.findUnique({ where: { id } });
  if (!entry) throw AppError.notFound('Entrada de diário não encontrada');
  if (entry.userId !== userId) throw AppError.forbidden('Você só pode editar seu próprio diário');
...
export const deleteEntry = async (userId: string, id: string) => {
  const entry = await prisma.diaryEntry.findUnique({ where: { id } });
  if (!entry) throw AppError.notFound('Entrada de diário não encontrada');
  if (entry.userId !== userId) throw AppError.forbidden('Você só pode remover seu próprio diário');
